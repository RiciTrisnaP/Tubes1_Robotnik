...
# This bot use the nearest distance to avoid zero division error in botv2  and also a fork of v3 but has diamond nearest distance from all bot checking

##### BUGGGGYYYYYYYYYYYYYY


## often stuck in base and idk :D

class Botv4(BaseLogic):
    def __init__(self):
    ...
    def select_target(self, diamond, board: Board, current_position, max_allow_point):
        ...
        min_distance = 100
        bots = [d for d in board.game_objects if d.type == "BotGameObject"];
        Reset_Button = [d for d in board.game_objects if d.type == "DiamondButtonGameObject"];

        # find highest weight diamond and check if it is nearest from all bots
        for i in diamond:
            ...
            if distance_to_candidate < min_distance and i.properties.points <= max_allow_point:
                is_nearest_from_all_bots = True
                for j in bots:
                    other_bot_distance = self.calculate_distance(j.position, i.position, i.properties.points)
                    if other_bot_distance < distance_to_candidate:
                        is_nearest_from_all_bots = False
                        break
                if is_nearest_from_all_bots:
                    min_distance = distance_to_candidate
                    target_position = i.position
                    self.target_type = 'DiamondGameObject'
                else:
                    target_position = Reset_Button[0].position
                    
        # if reset button has more weight then go to the button instead
        if self.calculate_distance(Reset_Button[0].position, current_position, 1) < min_distance:
            target_position = Reset_Button[0].position
